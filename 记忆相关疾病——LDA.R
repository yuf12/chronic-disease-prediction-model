...
library(candisc)
X <- train_data[, 1:9]
mlm_model <- lm(as.matrix(X) ~ memory_problem, data = train_data)
candisc_result <- candisc(mlm_model, term = "memory_problem")
...
# ------------------------------------------------------------
# 确保因变量为因子（二分类或多分类）
train_data$memory_problem <- as.factor(train_data$memory_problem)
test_data$memory_problem  <- as.factor(test_data$memory_problem)
...
# 2. 训练 LDA 模型
# ------------------------------------------------------------
lda_model <- lda(memory_problem ~ ., data = train_data)
...
library(candisc)
X <- train_data[, 1:15]
mlm_model <- lm(as.matrix(X) ~ Psyche, data = train_data)
candisc_result <- candisc(mlm_model, term = "Psyche")
print(candisc_result)
summary(candisc_result)   # 包含 Wilks' Lambda 检验详细表格
...
# 混淆矩阵（训练集）
cat("\n========== 训练集混淆矩阵 ==========\n")
train_cm <- confusionMatrix(train_pred$class, train_data$memory_problem)
print(train_cm)

# 混淆矩阵（测试集）
cat("\n========== 测试集混淆矩阵 ==========\n")
test_cm <- confusionMatrix(test_pred$class, test_data$memory_problem)
print(test_cm)
...
roc_train <- roc(train_data$memory_problem, train_pred$posterior[, 2])
cat("\n训练集 AUC:", round(auc(roc_train), 4), "\n")

roc_test <- roc(test_data$memory_problem, test_pred$posterior[, 2])
...
# 绘制ROC曲线
roc_plot_memory_problem <- ggplot(roc_data, aes(x = 1 - Specificity, y = Sensitivity, color = Dataset)) +
  geom_line(size = 1.2) +
