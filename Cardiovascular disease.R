...
dummy_model <- dummyVars(~ ., 
                         data = train_data[,3:14], 
                         fullRank = FALSE)

# 应用到训练集和测试集
train_encoded <- predict(dummy_model, train_data[,3:14])
test_encoded <- predict(dummy_model, test_data[,3:14])

# 合并编码后的部分与其他列
train_final <- cbind(
  train_data[, -c(3:14)],  # 保留除2-11列外的其他列
  train_encoded
)

test_final <- cbind(
  test_data[, -c(3:14)],   # 保留除2-11列外的其他列
  test_encoded
)
...
# 创建DMatrix
dtrain <- xgb.DMatrix(data = as.matrix(train_matrix[, -3]), label = train_data$heart_disease)
dtest <- xgb.DMatrix(data = as.matrix(test_matrix[, -3]), label = test_data$heart_disease)
...
refined_params <- list(
  max_depth = c(max(3L, random_results$best_params$max_depth - 2L), 
                min(10L, random_results$best_params$max_depth + 2L)),
...
roc_train <- roc(train_data$heart_disease, train_pred)
auc_train <- auc(roc_train)
ci_train <- ci.auc(roc_train)

# 计算测试集AUC
roc_test <- roc(test_data$heart_disease, test_pred)
