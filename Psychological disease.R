...
# 1. 数据准备
train_data <- read_excel("C:/Users/16039/Desktop/单病外部验证/情感训练集.xlsx")
test_data <- read_excel("C:/Users/16039/Desktop/单病外部验证/情感测试集.xlsx")
...
dummy_model <- dummyVars(~ ., 
                         data = train_data[,1:14], 
                         fullRank = FALSE)

# 应用到训练集和测试集
train_encoded <- predict(dummy_model, train_data[,1:14])
test_encoded <- predict(dummy_model, test_data[,1:14])

# 合并编码后的部分与其他列
train_final <- cbind(
  train_data[, -c(1:14)],  # 保留除2-11列外的其他列
  train_encoded
)

test_final <- cbind(
  test_data[, -c(1:14)],   # 保留除2-11列外的其他列
  test_encoded
)
...
# 创建DMatrix
dtrain <- xgb.DMatrix(data = as.matrix(train_matrix[, -2]), label = train_data$Psyche)
dtest <- xgb.DMatrix(data = as.matrix(test_matrix[, -2]), label = test_data$Psyche)
...
random_search_xgb <- function(n_iter = 50) {
  ...
  # 修改后的参数空间 - 注重正则化和防止过拟合
  search_space <- list(
    max_depth = c(3L, 8L),  # 降低最大深度范围
    min_child_weight = c(5L, 50L),  # 提高最小值，防止过拟合
    colsample_bytree = c(0.6, 0.9),  # 缩小范围，避免特征使用过低
    subsample = c(0.7, 0.9),  # 提高子采样比率下限
    gamma = c(0, 5),  # 增加gamma上限，增强正则化
    alpha = c(0, 10),  # 降低L1正则化上限
    eta = c(0.01, 0.2),  # 降低学习率范围
    nrounds = c(100L, 1000L),  # 保持足够迭代空间
    lambda = c(1, 20)  # L2正则化，设置合理范围
  )
...
# 根据随机搜索结果确定精细搜索范围
refined_params <- list(
  max_depth = c(max(3L, random_results$best_params$max_depth - 1L), 
                min(10L, random_results$best_params$max_depth + 1L)),
  
  min_child_weight = c(max(3L, random_results$best_params$min_child_weight * 0.8), 
                       min(60L, random_results$best_params$min_child_weight * 1.2)),
  
  colsample_bytree = c(max(0.5, random_results$best_params$colsample_bytree - 0.1), 
                       min(0.95, random_results$best_params$colsample_bytree + 0.1)),
  
  subsample = c(max(0.65, random_results$best_params$subsample - 0.1), 
                min(0.95, random_results$best_params$subsample + 0.1)),
  
  gamma = c(max(0, random_results$best_params$gamma - 2), 
            min(8, random_results$best_params$gamma + 2)),
  
  alpha = c(max(0, random_results$best_params$alpha - 3), 
            min(15, random_results$best_params$alpha + 3)),
  
  eta = c(max(0.005, random_results$best_params$eta * 0.7), 
          min(0.3, random_results$best_params$eta * 1.3)),
  
  nrounds = c(max(50L, random_results$best_params$best_iter - 30L), 
              min(1200L, random_results$best_params$best_iter + 100L)),
  
  lambda = c(max(0.5, random_results$best_params$lambda - 5), 
             min(25, random_results$best_params$lambda + 5))
)
...
final_params <- list(
  ...
  objective = "binary:logistic",
  eval_metric = "auc",
  max_depth = as.integer(7),
  min_child_weight =4.688805 ,
  colsample_bytree = 0.5502585,
  subsample = 0.6957404,
  gamma = 0.6117566,
  alpha =3.0160478,
  eta = 0.1669546,
...
roc_train <- roc(train_data$Psyche, train_pred)
auc_train <- auc(roc_train)
ci_train <- ci.auc(roc_train)

# 计算测试集AUC
roc_test <- roc(test_data$Psyche, test_pred)
