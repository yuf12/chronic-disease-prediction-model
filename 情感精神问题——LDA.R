...
# ------------------------------------------------------------
# 确保因变量为因子（二分类或多分类）
train_data$Psyche <- as.factor(train_data$Psyche)
test_data$Psyche  <- as.factor(test_data$Psyche)
...
# 2. 训练 LDA 模型
# ------------------------------------------------------------
lda_model <- lda(Psyche ~ ., data = train_data)
...
# 混淆矩阵（训练集）
cat("\n========== 训练集混淆矩阵 ==========\n")
train_cm <- confusionMatrix(train_pred$class, train_data$Psyche)
print(train_cm)

# 混淆矩阵（测试集）
cat("\n========== 测试集混淆矩阵 ==========\n")
test_cm <- confusionMatrix(test_pred$class, test_data$Psyche)
print(test_cm)
...
roc_train <- roc(train_data$Psyche, train_pred$posterior[, 2])
cat("\n训练集 AUC:", round(auc(roc_train), 4), "\n")

roc_test <- roc(test_data$Psyche, test_pred$posterior[, 2])
