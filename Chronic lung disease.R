...
# 应用到训练集和测试集
train_encoded <- predict(dummy_model, train_data[,2:11])
test_encoded <- predict(dummy_model, test_data[,2:11])

# 合并编码后的部分与其他列
train_final <- cbind(
  train_data[, -c(2:11)],  # 保留除2-11列外的其他列
  train_encoded
)

test_final <- cbind(
  test_data[, -c(2:11)],   # 保留除2-11列外的其他列
  test_encoded
)
...
# 创建DMatrix
dtrain <- xgb.DMatrix(data = as.matrix(train_matrix[, -1]), label = train_data$lung_disease)
dtest <- xgb.DMatrix(data = as.matrix(test_matrix[, -1]), label = test_data$lung_disease)
...
roc_train <- roc(train_data$lung_disease, train_pred)
auc_train <- auc(roc_train)
ci_train <- ci.auc(roc_train)

# 计算测试集AUC
roc_test <- roc(test_data$lung_disease, test_pred)
